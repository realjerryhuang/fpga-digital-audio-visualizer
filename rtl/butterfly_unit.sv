module butterfly_unit #(
	...
	// A + truncated (B * W)
    assign X0_re = A_re + $signed(WxB_re[WIDTH-2:WIDTH/2-1]);
    assign X0_im = A_im + $signed(WxB_im[WIDTH-2:WIDTH/2-1]);
    assign X1_re = A_re - $signed(WxB_re[WIDTH-2:WIDTH/2-1]);
    assign X1_im = A_im - $signed(WxB_im[WIDTH-2:WIDTH/2-1]);
    
    // Combine real and imaginary components of output signals
    assign X0 = {X0_re, X0_im};
    assign X1 = {X1_re, X1_im};
